...
describe('Households + Power Plants', function () {
  ...
  describe('complex network', () => {
    forEach([[10], [100]]).it('Complex network %d', (size) => {
      const world = new World();
      const households = new Array(size)
        .fill(undefined)
        .map(() => world.createHousehold());
      ...
      world.connectHouseholdToPowerPlant(
        households[getRandomInt()],
        powerPlant1,
      );
      world.connectHouseholdToPowerPlant(
        households[getRandomInt()],
      ...
      assert.equal(
        world.householdHasElectricity(households[getRandomInt()]),
        true,
      );
      assert.equal(
        world.householdHasElectricity(households[getRandomInt()]),
